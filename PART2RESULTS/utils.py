...
def part2Plots(results, save_dir="", filename="", show_plot=True):
    ...
    plot_curve_args = [
        {"c": color_list[k], "linestyle": style_list[0], "linewidth": 2}
    ...
    for result, args in zip(results, plot_curve_args):
        ax.plot(
            np.arange(1, len(result["loss_curve"]) + 1),
            result["loss_curve"],
            label=result["name"],
            **args
        )
    ...
    for result, args in zip(results, plot_curve_args):
        ax.plot(
            np.arange(1, len(result["train_acc_curve"]) + 1),
        ...
        ax.plot(
            np.arange(1, len(result["val_acc_curve"]) + 1),
        ...
        legend_elements = [
            Line2D([0], [0], color="k", linestyle=style_list[0], lw=2, label="train."),
...
def part3Plots(results, save_dir="", filename="", show_plot=True):
    ...
    sigmoid_curve_args = [
        {"c": color_list[k], "linestyle": style_list[1], "linewidth": 2}
        for k in range(num_results)
    ]
    ...
    legend_elements = [
        Line2D([0], [0], color="k", linestyle=style_list[0], lw=2, label="ReLU"),
    ...
    for result, relu_args, sigmoid_args in zip(
        results, relu_curve_args, sigmoid_curve_args
    ):
        ax.plot(
            np.arange(1, len(result["relu_loss_curve"]) + 1),
            result["relu_loss_curve"],
            label=result["name"],
            **relu_args
        )
        ax.plot(
            np.arange(1, len(result["sigmoid_loss_curve"]) + 1),
            result["sigmoid_loss_curve"],
            label=result["name"],
            **sigmoid_args
        )
        ax.set_xlabel(xlabel="step", fontsize=font_size)
    ...
    for result, relu_args, sigmoid_args in zip(
        ...
        ax.plot(
            np.arange(1, len(result["relu_grad_curve"]) + 1),
            result["relu_grad_curve"],
            label=result["name"],
            **relu_args
        )
        ax.plot(
            np.arange(1, len(result["sigmoid_grad_curve"]) + 1),
            result["sigmoid_grad_curve"],
            label=result["name"],
            **sigmoid_args
        )
        ax.set_xlabel(xlabel="step", fontsize=font_size)
...
def part4Plots(result, save_dir="", filename="", show_plot=True):
    ...
    fig.suptitle(
        "training of <%s> with different learning rates" % result["name"],
        fontsize=font_size,
        y=0.025,
    )
    ...
    for key_suffix, plot_args in zip(key_suffixes, plot_args):
        ...
        axes[0].plot(
            np.arange(1, len(loss_curve) + 1), loss_curve, label=label, **plot_args
        )
        ...
        axes[1].plot(
            np.arange(1, len(acc_curve) + 1), acc_curve, label=label, **plot_args
        )
...
def visualizeDataset(
    images, labels, save_dir, filename="dataset", num_samples_per_class=8
):
    ...
    for r in range(num_classes):
        ...
        for n in range(num_samples_per_class):
            axes[r, n].matshow(
                images[sample_indcs[n]].reshape(28, 28), cmap=plt.cm.gray
            )
            axes[r, n].set_xticks(())
            axes[r, n].set_yticks(())
